---
title: 'Fuzzy and Property-Based Clustering for Sequence Analysis: R Code introduction'
author: "Matthias Studer"
output: rmarkdown::html_vignette
bibliography: [packages.bib,manual.bib]
vignette: >
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteIndexEntry{Fuzzy and Property-Based Clustering for Sequence Analysis: R Code introduction}
...
Descriptive statistics of membership strength to each cluster provide useful informations. The mean can be interpreted as a relative frequency of each cluster, if sequences are weighted according to their membership strength.
...
Dirichlet regression are available in the `DirichletReg` package [@R-DirichletReg]. Here is a sample regression with sex and birth year as independent variables. 
...
Beta regression are  are available in the `betareg` package [@R-betareg]. To estimate a beta-regression for the third fuzzy type, one can use.
...
Second, the clustering algorithm is run. In `R`, the two steps are regrouped in the same function `seqpropclust`. The first argument specifies the state sequence object. Then, the `diss` argument specify the distance matrix, `maxcluster` the maximum number of cluster to consider, and `properties` the list of properties to consider using the names in the above table. 
...
The cluster quality indices can be computed using the `as.clustrange()` function as for hierarchical clustering algoriths. 
