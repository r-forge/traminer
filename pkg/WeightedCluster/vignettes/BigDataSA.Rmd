---
title: 'Sequence Analysis Typologies for Large Databases: R Code introduction'
author: "Matthias Studer"
output: rmarkdown::html_vignette
bibliography: manual.bib 
vignette: >
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteIndexEntry{Sequence Analysis Typologies for Large Databases: R Code introduction}
...
**Warning!!** To avoid lengthy computations (and overloading the CRAN server), we restricted the number of iterations and strongly recommend using higher values. 
...
bfclara <- seqclararange(biofam.seq, R = 100, sample.size = 200, kvals = 2:10, 
						 seqdist.args = list(method = "LCS"), parallel=TRUE, 
                         stability=TRUE)
```


The function requires us to specify the sequence to cluster (our `biofam.seq` sequence object), the number of iterations (argument `R`, here set to 100 to avoid long computation time, larger values are recommended) and the subsample size (argument `sample.size`, here again set to the low value of 200 for illustrative purpose). The number of groups in our typology is set using the `kvals` arguments, here between 2 and ten groups solutions. We directly specify a range of values that will be considered later on. Finally, we need to specify how to compute the distance between sequences through the `seqdist.args` argument as a `list` object. All the arguments specified here will be directly passed to the `seqdist` function. Therefore, any distance measures available in `seqdist` can be used here. Finally, we set `stability=TRUE` to estimate the stability of the clusterings among the subsamples.
...
The `bootclustrange` function can be used to bootstrap the cluster quality measures. It has the following arguments. The first object is the clustering to be evaluated, as a `seqclararange` object, a `data.frame` or a vector. Second, the sequences that were used to create the typology. We should further specify the distance measure to use (as before), the number of bootstraps (`R` argument), and the subsample size (here 500). We would generally use higher values for this last two arguments. Finally, the `parallel` and `progressbar` arguments could be used as before. The resulting object is then printed and the standardized value of the CQI are plotted. The results lead to the same conclusion as for the medoid-based CQI. 


```{r bcqi}
	bCQI <- bootclustrange(bfclara, biofam.seq, seqdist.args = list(method = "LCS"), R = 100, sample.size = 500,  parallel=TRUE)
	bCQI
...
```{r seqclarafuzzy, warning=FALSE, message=FALSE}
bfclaraf <- seqclararange(biofam.seq, R = 100, sample.size = 200, kvals = 2:10, method="fuzzy",
							seqdist.args = list(method = "LCS"), parallel=TRUE)
bfclaraf
...
```{r seqdplotclustf, dev="png", dpi=600}
