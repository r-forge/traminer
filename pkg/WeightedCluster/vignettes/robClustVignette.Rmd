...
```{r figSeqMvad, message=FALSE, fig.cap='MVAD trajectories, sorted from start'}
...
```{r seqdist, message=FALSE, time_it=TRUE}
...
```{r consClustTypo, message=FALSE, time_it=TRUE}
...
```{r plotConsClustCqi, message=FALSE, fig.cap="PAM and Ward consensus clustering CQIs (normalized)"}
# Plotting CQIs
par(cex = 0.75)
...
```{r consClustSeqplot, message=FALSE, fig.cap= "Crisp consensus Clustering in nine groups (PAM and Ward)"}
...
```{r consClustTypoFuzzy, message=FALSE, time_it=TRUE}
# Creating the typology
set.seed(1234)
...
```{r plotConsFuzzy, message=FALSE, fig.cap="Fuzzy consensus Clustering in nine groups (PAM and Ward), sorted by membership probability"}
par(mar = c(2,2,2,2))
...
```{r noiseTypo, message=FALSE, time_it=TRUE}
...
```{r plotNoiseClustCqi, message=FALSE, fig.cap="Fuzzy noise clustering CQIs, lambda = 0.8"}
# Plotting CQIs
plot(noiseClust08, 
     legendpos = "topleft")
```
...
```{r crispNoiseTypo, message=FALSE, fig.width=8, fig.height=5}
...
```{r noiseTypo06, message=FALSE, time_it=TRUE}
# Creating a typology with noise
set.seed(1234)
...
```{r noiseTypo1, message=FALSE, fig.width=8, fig.height=5, time_it=TRUE}
# Creating a typology with noise
set.seed(1234)
...
```{r crispNoiseTypo10, echo=FALSE, message=FALSE, fig.cap= "Crisp noise clustering in ten groups, lambda = 0.8 (delta = 68.39)"}
par(mar = c(2,2,2,2))
seqIplot(mvad.seq, 
...
```{r ngroupDnoiseCalc, echo= FALSE, message=FALSE}
...
```{r ngroupDnoise, message=FALSE, fig.cap= "Crisp noise clustering in ten groups, delta = 62.57"}
par(mar = c(2,2,2,2))
seqIplot(mvad.seq, 
