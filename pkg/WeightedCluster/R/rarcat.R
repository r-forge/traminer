...
rarcat <- function(bootout, clustering, clusnb, assoc, transformation=FALSE) {
  
  # Ensure correct bootout object
  if(!all(names(bootout) == c("B", "optimal.number", "cluster.solution", "assoc.char",
                              "original.result", "coefficients", "errors"))) {
